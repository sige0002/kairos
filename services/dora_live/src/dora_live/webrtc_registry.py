...
Ported from ``webrtc_streamer.registry`` (self-contained). Owns the live streams
(at most one per topic/encoding), each pairing a
:class:`~dora_live.webrtc_frame.FrameSource` (router buffer for one topic) with a
:class:`~dora_live.webrtc_peer.PeerManager` (per-client PeerConnections sharing
that buffer). Enforces deterministic ``stream_id`` per topic (a duplicate start
returns the existing stream) and idle auto-stop of an unreferenced stream after
``idle_timeout_s``. Depends only on the ``FrameSource`` / ``PeerManager`` seams
via injected factories, so its start/stop/idle/status logic is unit-testable
with fakes.
"""
...
from dora_live.webrtc_frame import FrameSource
...
from dora_live.webrtc_peer import PeerManager
...
# Factory signatures injected by the app factory (real) or tests (fakes).
...
class StreamRegistry:
    ...
    def start(self, request: StreamStartRequest) -> str:
        ...
        # Attach the source to the bus outside the lock.
        source.start()
        with self._lock:
            stream.state = StreamState.live
        logger.info(
            "stream started",
            extra={"component": "dora_live.webrtc", "topic": request.topic},
        )
        return sid
    ...
    async def stop(self, stream_id: str) -> bool:
        ...
        logger.info(
            "stream stopped",
            extra={"component": "dora_live.webrtc", "topic": stream.topic},
        )
        return True
