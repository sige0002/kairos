...
/// Largest element count any array may declare or carry. Real ROS messages are
/// orders of magnitude below this; the cap exists so that a malformed schema
/// (`Empty[4294967295]`, whose elements consume no bytes at all) cannot ask the
/// decoder to loop for hours over a payload of nothing.
pub const MAX_ARRAY_ELEMENTS: usize = 1 << 20;

/// Smallest number of bytes an encoded value of this type can occupy. This is
/// zero for a message with no fields, which is exactly the case where a
/// declared element count cannot be sanity-checked against the bytes left.
pub fn min_serialized_size(ft: &FieldType, defs: &[MsgDef]) -> usize {
    match ft {
        FieldType::Prim(p) => match p {
            ...
            Prim::Str | Prim::WStr => 4,
        },
        FieldType::Complex(i) => defs[*i]
            .fields
            .iter()
            .map(|(_, f)| min_serialized_size(f, defs))
            .sum(),
        FieldType::Array(inner, ArrayKind::Fixed(n)) => {
            min_serialized_size(inner, defs).saturating_mul(*n)
        }
        FieldType::Array(_, ArrayKind::Unbounded) => 4,
    }
}
    ...
    fn resolve_type(&mut self, tok: &str, pkg: &str, depth: usize) -> Result<FieldType> {
        ...
        if let Some(p) = tok.find('[') {
            ...
            let kind = if dim.is_empty() || dim.starts_with("<=") {
                ...
                let n: usize = dim.parse()?;
                if n > MAX_ARRAY_ELEMENTS {
                    bail!("fixed array dimension {n} in `{tok}` exceeds the {MAX_ARRAY_ELEMENTS} element limit");
                }
                ArrayKind::Fixed(n)
            };
