...
pub struct ReaderOptions {
    ...
    /// only decode these topics; `None` (the default) decodes every topic.
    /// Filtering happens before the message is decoded, so a consumer that
    /// wants a few topics out of a large bag does not pay for the rest —
    /// on a 29-topic rosbag where 2 topics are wanted this is most of the
    /// reader's work.
    pub topics: Option<HashSet<String>>,
    /// Stop at the first message that fails to decode instead of dropping the
    /// topic and reading on. The default (`false`) keeps the reader lossy but
    /// complete: a corrupt topic never blocks the rest of the bag, and the loss
    /// is reported in [`Stats::failed_topics`]. Set this when a caller must not
    /// silently receive a partial topic.
    pub strict: bool,
}
...
pub struct Stats {
    pub messages: u64,
    pub skipped: u64,
    /// messages skipped before decoding because of `ReaderOptions::topics`
    pub filtered: u64,
    pub topics: usize,
    /// a topic name carried by more than one channel, with a differing schema
    /// or encoding. Each channel is decoded with its own schema, but a consumer
    /// merging batches by topic name will see two different layouts
    /// (topic, first type, later type)
    pub ambiguous_topics: Vec<(String, String, String)>,
...
struct TopicState {
    /// the channel's topic name, kept here because state is keyed by channel id
    topic: String,
    schema: SchemaRef,
    kind: Kind,
    ts_log: Vec<i64>,
    ...
    fn push(&mut self, data: &[u8], log_time: u64, publish_time: u64) -> Result<()> {
        ...
        self.ts_pub.push(
            i64::try_from(publish_time)
                .context("publish_time is beyond the range of an Arrow timestamp")?,
        );
...
pub struct McapArrowReader<'a> {
    ...
    /// Keyed by channel id, not topic name: two channels may carry the same
    /// topic with different schemas, and each must be decoded with its own.
    /// Hashing a u16 per message is also far cheaper than hashing the topic.
    channels: HashMap<u16, TopicState>,
    /// channel ids excluded by `ReaderOptions::topics`, decided once per channel
    skip: HashSet<u16>,
    /// topic name -> message type of the channel that first claimed it
    seen_topics: HashMap<String, String>,
    opts: ReaderOptions,
    drain: Vec<u16>,
    stats: Stats,
}
...
impl<'a> McapArrowReader<'a> {
    ...
    pub fn next_batch(&mut self) -> Result<Option<TopicBatch>> {
        while let Some(stream) = self.stream.as_mut() {
            let Some(message) = stream.next() else {
                // end of stream: queue non-empty channels for the final flush
                self.stream = None;
                self.drain = self
                    .channels
                    .iter()
            ...
            if self.skip.contains(&cid) {
                self.stats.filtered += 1;
                continue;
            }
            if !self.channels.contains_key(&cid) {
                if let Some(wanted) = &self.opts.topics {
                    if !wanted.contains(&m.channel.topic) {
                        self.skip.insert(cid);
                        self.stats.filtered += 1;
                        continue;
                    }
                }
                ...
                match self.seen_topics.get(&m.channel.topic) {
                    Some(first) if *first != st.msg_type => {
                        self.stats.ambiguous_topics.push((
                    ...
                    None => {
                        self.seen_topics
                            .insert(m.channel.topic.clone(), st.msg_type.clone());
                    }
                }
                self.stats.topics += 1;
                self.channels.insert(cid, st);
            }
            let st = self
                .channels
                .get_mut(&cid)
                .expect("channel state inserted above");
            if st.failed {
        ...
        while let Some(cid) = self.drain.pop() {
            let st = self
                .channels
                .get_mut(&cid)
                .expect("drain ids come from channels");
    ...
    /// If more than one channel carries the topic, this is the first one's —
    /// see [`Stats::ambiguous_topics`].
