...
- `topics: Some(...)` で購読するトピックだけをデコードする。**フィルタはデコードの前**に
  かかるので、29トピックのbagから2トピックだけ読む実測でスキャンは 0.28s → 0.076s。
  弾いた件数は `Stats::filtered` に出る
- 既定ではトピック単位のデコード失敗で**止まらずに読み進める**。壊れた1トピックが
  bag全体の読み出しを潰さないための設計で、**損失は必ず `Stats::failed_topics` に出る
  ので、呼び出し側はこれを見ること**。部分的なデータを受け取ってはいけない用途では
  `strict: true` にすると最初の失敗でエラーを返す
- 同じトピック名を別スキーマの複数チャンネルが持つ場合、各チャンネルは自分のスキーマで
  正しくデコードされ、その事実が `Stats::ambiguous_topics` に出る
  (トピック名でバッチをマージする消費者は2種類のレイアウトを見ることになる)
