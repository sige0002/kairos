...
Everything here is read-only and non-intrusive: ``/proc`` reads, one ``shutil``
stat, and short-timeout ``nvidia-smi`` runs. Nothing touches the ROS 2 graph, so
a probe can never disturb a recording.

**No probe raises.** A missing ``/proc``, an absent ``nvidia-smi``, a timeout, a
parse error, a data dir that is not there — each degrades its own field to
``None`` so the UI can honestly show "—" for what it could not measure, instead
of the endpoint failing as a whole because one number was unavailable.

Two results are cached on ``app.state`` rather than recomputed per request: the
GPU NAME is immutable for the process lifetime and is probed at most once, and
the utilization sample is refreshed at most every :data:`_SAMPLE_TTL_S` seconds
so the UI's ~5s polling cannot turn into a subprocess per request. The state
object is passed in rather than reached for, which is what keeps these functions
callable without an app.
"""
