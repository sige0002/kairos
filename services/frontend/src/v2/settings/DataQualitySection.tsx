...
export function DataQualitySection({ config }: { config: RuntimeConfig | undefined }) {
  ...
  const robotQuery = useQuery({
    ...
    queryFn: ({ signal }) => getRobotConfig(active!, { signal }),
    enabled: !!active,
  });
  const robotCfg = robotQuery.data;

  const rec = (robotCfg?.aspects.recording?.content ??
    null) as RecordingContentView | null;
  const val = (robotCfg?.aspects.validation?.content ??
    null) as ValidationContentView | null;
  ...
  return (
    <div
      className="flex min-w-0 flex-col gap-2.5 overflow-auto lg:col-span-2 lg:min-h-0"
    ...
    >
      <Card className="flex min-w-0 flex-col gap-5 p-[18px]">
        ...
        {optionsQuery.isError ? (
          ...
          <>
            <Section title={t('dataQuality.expectedRates')}>
              {patterns.length === 0 ? (
                <p className="text-[12.5px] text-text-muted">
                  {t('dataQuality.noExpectedRates')}
                </p>
              ) : (
                <div
                  className="overflow-hidden rounded-control border border-border"
                  data-testid="dq-expected-hz"
                >
                  {patterns.map((p) => (
                    <div
                      key={p.pattern}
                    ...
                    >
                      ...
                      <span className="font-mono text-text-secondary">
                        {p.hz != null ? `${p.hz} Hz` : t('dataQuality.noExpectedRates')}
                      </span>
                    </div>
                  ))}
                </div>
            ...
            <Section title={t('dataQuality.healthThresholds')}>
              <p
                className="text-[12.5px] leading-relaxed text-text-secondary"
                data-testid="dq-thresholds"
              >
                {t('dataQuality.thresholdDescription', {
                  warn: pct(warn, 0.02),
                  danger: pct(danger, 0.05),
                })}
              </p>
            </Section>

            <Section
              title={t('dataQuality.validationTopics', {
                count: requiredTopics.length,
              })}
            >
              {requiredTopics.length === 0 ? (
                <p className="text-[12.5px] text-text-muted">
                  {t('dataQuality.noRequiredTopics')}
                </p>
              ) : (
                <ul
                  className="max-h-64 overflow-auto rounded-control border border-border text-[12px]"
                  data-testid="dq-required-topics"
                >
