...
export function SignalsView() {
  ...
  const statusKey = status === 'open' ? 'live' : status;
      ...
      <Card className="flex shrink-0 flex-col">
        <div className="flex flex-wrap items-end gap-3 px-[18px] py-3.5">
          ...
          <label className="flex flex-col gap-1">
            ...
            <select
              ...
              aria-label={t('signals.fieldLabel')}
              value={addField ?? ''}
              ...
              <option value="">
                {!addTopic
                  ...
                  : fieldsQuery.isPending
                    ? t('signals.introspecting')
                    : fields.length === 0
                      ? t('signals.noFields')
                      : t('signals.selectField')}
              </option>
              {fields.map((f) => (
          ...
          <div className="ml-auto flex items-end gap-3">
            ...
            <label className="flex flex-col gap-1">
              ...
              <select
                aria-label={t('signals.rateLabel')}
                value={hz}
            ...
            >
              {live ? t('signals.pause') : t('signals.resume')}
            </button>
          </div>
        </div>
          ...
          <div className="border-t border-border px-[18px] py-2.5">
            <div className="mb-1.5 flex items-center gap-2.5">
              <h3 className="text-[10.5px] font-semibold uppercase tracking-[0.05em] text-text-muted">
                {t('signals.series', { count: series.length })}
              </h3>
              <button
                type="button"
                onClick={clearSeries}
            ...
            <div className="flex flex-wrap gap-2">
              {series.map((s, i) => (
                ...
                >
                  ...
                  <button
                    type="button"
                    aria-label={t('signals.removeLabel', { series: seriesLabel(s) })}
                    title={t('signals.remove')}
        ...
        {distinctTopics > TOPIC_WARN && (
          <p className="border-t border-status-warning-border bg-status-warning-bg px-[18px] py-1.5 text-[11px] text-status-warning-text">
            {t('signals.warning', { count: distinctTopics, limit: String(TOPIC_WARN) })}
          </p>
        )}
      </Card>
        ...
        <div className="flex items-center gap-2.5 border-b border-border px-[18px] py-3">
          ...
          <span className="inline-flex items-center gap-1.5 text-[11.5px] text-text-muted">
            ...
            <span data-testid="signals-status" className="font-mono">
              {t(`signals.${statusKey}` as 'signals.live')}
            </span>
          </span>
        </div>
