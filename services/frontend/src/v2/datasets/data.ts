...
export type DatasetAddBlockReason = 'bytes_not_local' | 'not_adopted';
...
export function addBlockedReasons(capture: CaptureListItem): DatasetAddBlockReason[] {
  const reasons: DatasetAddBlockReason[] = [];
  if (!isCapturePresent(capture)) reasons.push('bytes_not_local');
  if (capture.review_status !== 'adopted') reasons.push('not_adopted');
  return reasons;
}
