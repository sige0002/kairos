...
// One batch: run_001 is still present (validatable); run_gone was exported.
const BATCHES = {
  items: [
    {
      batch_id: 'batch_x',
  ...
  vi.spyOn(globalThis, 'fetch').mockImplementation((input, init) => {
    ...
    if (url.match(/\/jobs\/[^/]+\/status/)) {
      ...
      return Promise.resolve(
        jsonResponse({ job_id: id, state: 'succeeded', progress: 1 }),
      );
    }
    if (url.match(/\/jobs\/[^/]+\/result/)) {
      const id = url.split('/jobs/')[1]?.split('/')[0] ?? 'j1';
      return Promise.resolve(
        jsonResponse(resultByRunId[id] ?? { summary: { result: 'pass' } }),
      );
    }
    if (url.endsWith('/jobs')) {
      ...
      resultByRunId[jobId] = resultByRunId[`for:${body.run_id}`] ?? {
        summary: { result: 'pass' },
      };
      return Promise.resolve(
...
test('clicking a preset runs its pipeline over exactly its pending_run_ids', async () => {
  ...
  expect((postedBodies.map((b) => b.run_id) as string[]).sort()).toEqual([
    'run_001',
    'run_002',
  ]);
...
test('the target selector offers exported datasets grouped separately from runs', async () => {
  ...
  expect(
    within(target).getByRole('group', { name: 'Runs (before export)' }),
  ).toBeInTheDocument();
  expect(
    within(target).getByRole('group', { name: 'Datasets (exported)' }),
  ).toBeInTheDocument();
  expect(
    await screen.findByRole('option', { name: '07/13 · #4 · pick' }),
  ).toBeInTheDocument();
  expect(
    screen.getByText('Validation only — export stays in Review.'),
  ).toBeInTheDocument();
});
...
test('a batch target validates every still-present run of that batch (blast radius)', async () => {
  renderWithClient(<ValidationScreen />);
  const target = (await screen.findByLabelText('target')) as HTMLSelectElement;
  expect(
    within(target).getByRole('group', {
      name: 'Batches (validate every run of a batch)',
    }),
  ).toBeInTheDocument();
  // The exported run is excluded from the candidate count, honestly.
  const option = await screen.findByRole('option', {
    name: /07\/13 · #4 · pick \(1 runs\)/,
  });
...
test('a dataset target disables run-only pipelines with an "applies to runs" note', async () => {
  ...
  await waitFor(() =>
    expect(within(fast).getByText('applies to runs')).toBeInTheDocument(),
  );
  expect(screen.getByRole('button', { name: 'Run on selection' })).toBeDisabled();
  // loss_report stays applicable (dataset-capable).
  expect(
    within(screen.getByTestId('pipeline-card-loss_report')).queryByText(
      'applies to runs',
    ),
  ).toBeNull();
});
