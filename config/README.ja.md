...
│  ├─ signals/default.yaml        # Review > Signals の既定表示（任意・S1'）
...
- **Settings > Data quality** から、選択式ではない単一ファイル設定 2 種を編集できる:
  `signals/default.yaml`（Review の既定表示。`GET/PUT /api/v1/config/signals`。表示専用＝即時反映）と
  `monitoring/alerts.yaml`（アラート規則。`GET/PUT /api/v1/config/alerts`。topic_monitor 再起動時に反映）。
  いずれもアクティブ機体のファイルへアトミックに書き戻す（詳細は `docs/specs/ja/api_orchestrator.md`）。
...
- `frontend`（UI）… `GET /api/v1/config` 経由で Record / Monitor の事前選択・バッジ、Stream の初期ペイン。
  Review > Signals の既定表示は `signals/default.yaml`（`GET /api/v1/config/signals`。表示専用＝即時反映）。
