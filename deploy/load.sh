...
# The monitor is the honest source for what the topics actually carry (measured,
# not derived from a nominal rate). Port follows the project convention
# (.env TOPIC_MONITOR_PORT) rather than a literal.
MON_PORT="${TOPIC_MONITOR_PORT:-8001}"
METRICS_TMP="$(mktemp -t kairos-load-metrics.XXXXXX)"
trap 'rm -f "$METRICS_TMP"' EXIT
if curl -sf -m 2 "http://localhost:${MON_PORT}/metrics" >"$METRICS_TMP" 2>/dev/null; then
    python3 - "$MON_PORT" "$METRICS_TMP" <<'PYEOF'
import json, sys
...
PYEOF
    ...
    # Say so rather than omitting the section: a silently missing line reads as
    # "no DDS traffic", which is a different claim from "nobody measured it".
    echo "== LIVE DDS  (monitor:${MON_PORT})  unreachable — no measurement"
fi
