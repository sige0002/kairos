...
const REASON = 'e2e: gripper never closed — unusable take';
...
test('§13-3 Discard: the modal requires a reason, the capture leaves the lists, the ledger keeps the tombstone', async ({
  ...
  await expect(confirm, 'discard armed with no reason typed').toBeDisabled();
  ...
  await page.getByTestId('discard-reason').fill(REASON);
  ...
  expect(JSON.stringify(events[0]), 'the operator reason was not kept').toContain(REASON);
  ...
  expect(tombstone!.delete_reason).toBe(REASON);
});
