...
- **`clock_check`** — ロボット非依存の**録画ホスト vs パブリッシャの時計整合性チェック**（ROS 不要・mcap 直読み）。bag 内の 2 つの時刻は別マシンの時計で打たれる — `header.stamp` はパブリッシャ（split 構成ではロボット側）、MCAP `log_time` は録画ホストの受信時刻 — ので、その差は本来伝送＋キューの遅延（ms オーダー）であり、録画 PC の時計が狂っていると差がそのまま時計オフセットになる（録画側が遅れていれば**負値**＝「発行前に受信」）。手法: ①decode 無しの全走査でトピックごとの `(log_time, publish_time)` と記録窓を取得し、`publish_time` が信頼できる場合（loss_report と同じ `source_times` 規則）は `publish_time − log_time` の中央値を **decode 不要のクロスチェック**として併記（header 差が大きいのに publish 差が小さければ「時計」でなくパブリッシャ側の遅い stamping）。②bag の**先頭窓と末尾窓**に分けて `max_samples_per_topic`（既定 200）まで decode し、`log_time − header.stamp` の中央値/p05/p95/min/max/負値率をトピックごとに算出（**全編 decode は絶対にしない** — 窓とクォータで有界）。判定: `|median| > threshold_ms`（既定 500）で `offset_suspected`、先頭窓と末尾窓の中央値が threshold 超で乖離すると `step_suspected`（**録画中の NTP ステップ**の形。log_time 側に偽ギャップ/時間逆行が刻まれる）。header の無い型・zero stamp のトピックは理由付きで報告し**フラグしない**（zero stamp を epoch と比較して 56 年のオフセットを捏造しない）。summary は `result: pass|fail`（fail = フラグ 1 件以上）を持ち Validation タブの汎用レンダラでゲートでき、「これは推定であり、どちらの時計が正しいかの測定ではない」旨の definition を明記する。params: `threshold_ms` / `max_samples_per_topic` / `target_topics`（glob）。レポート: `data/report/clock_check/<capture_id>/summary.json`。なお **bag 再生を再録画した capture は必ず fail する**（header.stamp は元収録時刻のまま再生されるため） — これは正しい検出であり、そうした capture が学習データに混ざる前に見える化される。
