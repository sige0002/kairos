...
| `KAIROS_ARCHIVE_ROOTS` | (任意・既定は空=無効) | capture の archive 先として許可するルート（`:` 区切り）。空なら archive 機能自体を提供しない（必ず失敗するボタンを置かない）。destination はここに対して検証され、`data_dir` と重なるものは拒否される（[capture_store](capture_store.md) §6） |
