...
- **Audio（端末単位、2026-08-27）**: master / Sound / Voice を独立設定し、イベント行ごとにも Sound / Voice を切り替える。language / voice / failure-reason 語彙を変えた後は `Prepare voice assets` で cache を更新する。engine unavailable と browser blocked を常設 status で区別し、Sound-only の回復可能性を示す。Preview は user gesture を兼ねる。
