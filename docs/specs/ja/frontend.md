...
- **トピック検索**: Topics のトピック表・追加チャートのトピック選択・Signals のトピック選択に検索欄を置く。トピック名の部分一致（大文字小文字を区別せず、前後の空白を除去）で候補を絞り込む。検索は録画対象・描画中の系列・選択済みのトピックとフィールドを変更しない。一致なしを明示し、「検索をクリア」で候補一覧を戻して検索欄へフォーカスを戻す。
- **Topics ビュー**:
...
- **Audio（端末単位、2026-08-27）**: master / Sound / Voice を独立設定し、イベント行ごとにも Sound / Voice を切り替える。Sound と Voice は別々の preview 操作とし、Sound の Web Audio player はスケジュール直後に close せず Audio 画面の unmount まで保持する。Voice は単一の local Kokoro 82M（英語・日本語、Apache-2.0）で、UI は language ごとの話者と `0.75..1.25` の話速を選べる。短い定型文には sidecar が終端句読点を補い、日本語は表示用の漢字をそのまま読み上げる。voice の作成は `Prepare voice assets` を明示したときだけで、language / voice / 話速 / failure-reason 語彙を変えると旧 asset を無効化し、同ボタンで更新する。recorder が `armed` / `recording` / `stopping` の間、recorder state を読めない間、または録画開始が作成を preempt した場合は作成を延期し、停止後に再試行する。engine unavailable、作成時の phrase 単位 failure、voice service 到達不能、browser blocked を status で区別し、いずれも Sound-only と Collect 継続の回復可能性を示す。Preview は user gesture を兼ねる。Audio は任意の補助であり、外部クラウド TTS、他端末への設定・asset 同期、録画中の TTS 作成は提供しない。旧 eSpeak / VOICEVOX の保存済み話者と asset は初回読込時にKokoro既定話者へ安全に移行し、再prepareを要求する。評価根拠は日本語漢字・英語の実モデル WAV 生成、話者/話速変更の request・cache identity test、engine 不在/録画優先の回復経路に対する expert review であり、代表利用者による聴感評価は未実施である。
  - **準備状態と pre-arm の競合回復（2026-09-02）**: Settings の `Prepare voice assets` は明示操作として、同じ `capture_id` の未開始 `armed` session だけを安全に解放してから生成する。照合までに録画が開始した場合や別端末が再び pre-arm した場合は録画を止めず準備を中断し、再試行条件を表示する。画面には現在の phrase 集合に対する `準備済み件数 / 必要件数` を常設し、部分準備を完了扱いしない。Collect のスピーカーも有効な Voice asset が不足していれば warning 状態と Settings への回復手順を示し、asset 不在を無言で隠さない。同一 origin の既に開いている別タブには `storage` event で master / event / asset 設定を反映するが、別ブラウザ・別端末へは従来どおり同期せず、音も操作元タブだけで鳴らす。
...
- **#57 の完了範囲**: `common`（shell・共通 action/error・global Store Health banner）、`monitor`（Overview / Topics / Signals / System / Store / Events / Logs）、`settings`（General / Collection / Data / Workspace / Notifications / Advanced）の Kairos 所有 UI 文言を EN / JA で提供する。表示 text だけでなく、`aria-label` / `aria-description` / `title` / placeholder、空・読取失敗・確認・復旧の文言も resource に置く。
- **resource の追加規則**: locale の正本 shape は `en` とし、`resources.test.ts` が対応する全 locale の leaf key path を完全一致で比較する。新しい Settings section は `settings` namespace の EN と JA に**同じ key を同じ変更で**加え、その section が使う key だけを追加する。無関係な namespace の移動・rename や、英語 mirror の手編集はしない。
- **inline copy guard**: `npm run lint` は TypeScript AST で JSX text、静的な `title` / `aria-label` / `aria-description` / `alt` / placeholder、`window.alert` / `confirm` / `prompt` の文字列・template・連結式を確認する。対象は shell、shared/store、Monitor、Settings、Collect / Review / Datasets / Validation を含む完了済みの全画面である。allowlist は path・sink・literal・**厳密な件数**の両方向比較であり、新しい literal だけでなく使われなくなった例外も失敗にする。guard 自身の deterministic self-test は「新規 literal」「古い例外」「動的値を含む dialog 文」を確認する。例外は記号、単位、protocol/configuration の例、言語名の endonym に限る。これは自然言語判定器ではないため、動的な user/server 値そのものは翻訳・検査しない。
- **データの境界**: Project / Task / Condition / Operator / dataset 名、topic・capture ID、設定の raw 内容、サーバーの warning / failure reason / corrupt sidecar reason などは翻訳・言い換えしない。周辺の説明と操作だけを翻訳し、診断の原文を残すことで復旧時に検索・照合できる。
- **QA 定義**: EN / JA × Light / Dark の 4 組で shell tab、Monitor の healthy / warning / SUSPECT 表示、Settings の General（Language / Appearance）と複雑 section（Recording / Audio / Alerts など）を semantics（role、label、testid、state）で確認する。locale 切替は現在の URL route・未保存の画面内 state・録画 state を保ち、locale を理由に mutation または refetch を送らない。Vitest は resource parity、Language/Appearance、Store healthy/SUSPECT と raw diagnostics を確認し、実スタック E2E は `10-settings.spec.ts` の active-recording safety、`09-monitor.spec.ts` の Monitor 実測、`05-missing-repair.spec.ts` の SUSPECT/Repair を組み合わせる。E2E は screenshot golden ではなく semantics を assertion し、実スタックを直列に起動できる時だけ実行する。
