...
- **Condition の文脈表示**: Review 右側の Inspection は、選択 capture の `batch_id` から現在の Batch の Condition を解決し、**Task の直下**に表示する。Condition は Batch 所有なので、この場では読み取り専用（変更は Collect の batch menu）。Batch が無い、または値が未設定なら「—」とし、capture に値を複製して補完しない。Capture API は immutable `collection_context` も露出するが、Review/Datasets の表示・検索をその snapshot に切り替えるのは **Phase 3 TBD** である。
