...
- Audio voice asset: `GET /api/v1/audio/status`、`POST /api/v1/audio/assets`、`GET /api/v1/audio/assets/{asset_id}.wav` — Settings だけが local CPU TTS（現行 `espeak-ng`、交換可能な provider 境界）を実行し、`catalog/audio/` に phrase/language/voice/engine の content-addressed WAV を atomic publish する。TTS request 同士だけを直列化し、phrase ごとの生成直前に recorder state を再確認する。recorder の `prepare` / `start` は TTS lock を待たず、同期的に recording priority を予約して進行中 subprocess を terminate し、cancel token により古い一時 WAV の publish も拒否して直ちに録画を優先する。recorder が `armed` / `recording` / `stopping`、または state を確認できない間は `deferred: true` と `errors` で延期を返し、録画負荷から fail-closed で隔離する。WAV 配信は cache 内の通常ファイルだけを symlink 非追従で読む。Collect はこの生成 API を呼ばず、事前取得した asset URL だけを browser で再生する。engine 不在・phrase 単位の生成失敗は `200` の `available/errors` で明示し、録画系 API の状態には影響しない。入力は `en|ja`、1 phrase 200 文字、1 request 64 phrases を上限とする。
- **Audio asset API 契約（任意・録画経路外）**:
  - `GET /api/v1/audio/status` は `{ available, engine, voices }` を返す。`engine` は provider 名または `null`、`voices` は利用可能な `en` / `ja` voice の一覧または空オブジェクトである。現行 runtime image はネットワークを使わない CPU-only `espeak-ng` を同梱するが、provider 発見に失敗すれば Voice は graceful に無効化される。
