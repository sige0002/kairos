...
- Audio voice asset: `GET /api/v1/audio/status`、`POST /api/v1/audio/assets`、`GET /api/v1/audio/assets/{asset_id}.wav` — Settings だけが local CPU TTS（既定の軽量 `espeak-ng`、任意の日本語 neural `voicevox`）を実行し、`catalog/audio/` に phrase/language/voice/engine の content-addressed WAV を atomic publish する。TTS request 同士だけを直列化し、phrase ごとの生成直前に recorder state を再確認する。recorder の `prepare` / `start` は TTS lock を待たず、同期的に recording priority を予約して進行中の subprocess / HTTP response を cancel し、cancel token により古い一時 WAV の publish も拒否して直ちに録画を優先する。recorder が `armed` / `recording` / `stopping`、または state を確認できない間は `deferred: true` と `errors` で延期を返し、録画負荷から fail-closed で隔離する。WAV 配信は cache 内の通常ファイルだけを symlink 非追従で読む。Collect はこの生成 API を呼ばず、事前取得した asset URL だけを browser で再生する。engine 不在・phrase 単位の生成失敗は `200` の `available/errors` で明示し、録画系 API の状態には影響しない。入力は `en|ja`、1 phrase 200 文字、1 request 64 phrases を上限とする。
- **Audio asset API 契約（任意・録画経路外）**:
  - `GET /api/v1/audio/status` は `{ available, engine, configured_provider, voices }` を返す。`engine` は実際に利用可能な provider 名または `null`、`configured_provider` は起動時選択、`voices` は利用可能な `en` / `ja` voice の一覧または空オブジェクトである。通常 image はネットワークを使わない CPU-only `espeak-ng` を同梱する。`TTS_PROVIDER=voicevox` では日本語を loopback の VOICEVOX、英語を eSpeak へ route する。構成した provider の発見に失敗すれば Voice は graceful に無効化し、別 provider へ暗黙に切り替えない。
  - `POST /api/v1/audio/assets` は `{ phrases: [{ key, text, language, voice }] }` を受け、`key` は英数字・`_.:-` の 1–80 文字、`text` は 1–200 文字、`language` は `en|ja`、`voice` は 1–40 文字、`phrases` は 1–64 件とする。成功・engine 不在・延期・phrase 単位の失敗はすべて `200` で `{ available, engine, assets, errors, deferred }` を返す。`assets[]` は `{ key, asset_id, url }`、`url` はこの API の `GET /api/v1/audio/assets/{asset_id}.wav` である。入力 validation と未対応 language / voice は `422`、不正 id または存在しない asset は `404`。
  - cache は `<data_dir>/catalog/audio/<sha256>.wav`。asset id は engine / language / voice / text を含む content address で、同一の通常ファイルは再生成しない。一時ファイルを同じ directory に作り、非空を確認して `os.replace` するため publish は atomic。配信は symlink をたどらず通常ファイルだけを読み、5 MiB 超も返さない。asset は browser-local Settings が URL を保持するだけで、サーバから参照を削除する API / TTL はない。
  - generation request は相互に直列化するだけで、`record/prepare` と `record/start` は lock を待たない。両 endpoint はまず recording priority を同期的に予約して epoch を単調増加させる。state 確認後に待機していた request を含め、古い epoch で admission された generation は予約が既に release 済みでも拒否する。進行中の synthesis は cancel token と subprocess terminate または HTTP response close で止め、cancel 後の一時 WAV は publish しない。`espeak-ng` が SIGTERM に応答しなくても SIGKILL への escalation は別 daemon thread で行うため、Prepare / Start の recorder path は待たない。
