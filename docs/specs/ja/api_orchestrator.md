...
- プラン語彙カタログ: `GET /api/v1/plans` / `PUT /api/v1/plans` — Collect がバッチ/エピソードに刻む **project / task / condition の共有語彙**。canonical 形は `projects: [{ project_id, name, tasks: [{ task_id, name, conditions: [{ condition_id, name }] }] }]`。各段の ID は rename/reorder をまたいで安定し、Batch/Start context は ID と表示名を保存する。GET は `revision` を返す（未設定は `projects: null, revision: 0`）。PUT は全置換 body に `base_revision` 必須で、同値でなければ `409 plans_conflict`（`current_revision` / `base_revision`）となり、last-writer-wins は行わない。ラベルは trim+Unicode NFC、空白・`—`・同階層の重複、ID 重複を拒否する。旧 DB/sidecar の name/string 形は rebuild で決定的 ID を付け canonical sidecar に書き戻す。`failure_reasons` は Collect の Failure 理由チップ、`operators` は帰属ロスター（認証ではない）で、いずれも同じ revision の catalog に載り、PUT で省略すれば保存済みの値を保持する。各 task は `failure_shortcuts: { left, center, right }` を持ち、スロットは共有 `failure_reasons` 語彙のラベルか `null`。設置共通の `external_controls` は schema version 1 と、`ready` / `recording` / `result` / `failure_reason` ごとの `{ left, center, right }` を持つ。許可アクションは順に Start、Stop、Success + Save / Failure / Retake、reason slot 1–3（各状態で `none` 可）だけで、同じ非 `none` アクションの重複、未知 action/state/slot、未知 schema version は 422。省略した旧クライアントの PUT は保存済み mapping を保持し、未設定 GET は `external_controls: null` を返して frontend が後方互換な既定 mapping を seed する。検証: 同一 task 内の重複理由は拒否（422）、割り当てられた理由は**今回の PUT が持てば提出・無ければ保存済みの** effective な語彙に属さないと `422 failure_shortcut_unknown_reason`（slot と reason を details で返す）。`failure_shortcuts` field 無し payload（追加前のカタログ）は読み取り時に全未割り当てへ正規化される（rebuild 耐性）。
...
- Audio voice asset: `GET /api/v1/audio/status`、`POST /api/v1/audio/assets`、`GET /api/v1/audio/assets/{asset_id}.wav` — Settings だけが単一の local CPU Kokoro 82M TTS を実行し、`catalog/audio/` に phrase/language/voice/speed/engine/model revision の content-addressed WAV を atomic publish する。TTS request 同士だけを直列化し、phrase ごとの生成直前に recorder state を再確認する。recorder の `prepare` / `start` は TTS lock を待たず、同期的に recording priority を予約して sidecar の cancel endpoint で inference worker を終了し、cancel token により古い一時 WAV の publish も拒否して録画を優先する。recorder が `armed` / `recording` / `stopping`、または state を確認できない間は `deferred: true` と `errors` で延期を返し、録画負荷から fail-closed で隔離する。WAV 配信は cache 内の通常ファイルだけを symlink 非追従で読む。Collect はこの生成 API を呼ばず、事前取得した asset URL だけを browser で再生する。engine 不在・phrase 単位の生成失敗は `200` の `available/errors` で明示し、録画系 API の状態には影響しない。入力は `en|ja`、話速 `0.75..1.25`、1 phrase 200 文字、1 request 64 phrasesを上限とする。
- **Audio asset API 契約（任意・録画経路外）**:
  - `GET /api/v1/audio/status` は `{ available, engine, model_revision, voices }` を返す。`engine` は利用可能なら `kokoro-82m`、それ以外は `null`、`model_revision` は固定 model revision または `null`、`voices` は利用可能な `en` / `ja` voice の一覧または空オブジェクトである。Kokoro 発見に失敗すれば Voice は graceful に無効化し、別 engine へ暗黙に切り替えない。sidecar が後から ready になった場合は次の status / prepare request で再発見するため、録画の起動条件にはしない。
  - `POST /api/v1/audio/assets` は `{ phrases: [{ key, text, language, voice, speed }] }` を受け、`key` は英数字・`_.:-` の 1–80 文字、`text` は 1–200 文字、`language` は `en|ja`、`voice` は 1–40 文字、`speed` は `0.75..1.25`（省略時 `1.0`）、`phrases` は 1–64 件とする。成功・engine 不在・延期・phrase 単位の失敗はすべて `200` で `{ available, engine, model_revision, assets, errors, deferred }` を返す。`assets[]` は `{ key, asset_id, url }`、`url` はこの API の `GET /api/v1/audio/assets/{asset_id}.wav` である。入力 validation と未対応 language / voice は `422`、不正 id または存在しない asset は `404`。
  - Settings の明示的な準備操作は上記 body に `release_prearm: true` を加える。この指定で recorder が `armed` の場合、orchestrator は recorder の原子的な compare-and-disarm にその `capture_id` を渡し、**同じ未開始 session だけ**を disarm してから音声を生成する。照合までに `recording` / `stopping` へ進んだ場合や別 capture に変わった場合は recorder が何も停止せず `disarmed: false` を返し、orchestrator は `deferred: true` とする。省略時は `false` で、既存 caller が pre-arm を解放することはない。
  - cache は `<data_dir>/catalog/audio/<sha256>.wav`。asset id は engine / model revision / language / voice / speed / text を含む content address で、同一の通常ファイルは再生成しない。一時ファイルを同じ directory に作り、非空を確認して `os.replace` するため publish は atomic。配信は symlink をたどらず通常ファイルだけを読み、5 MiB 超も返さない。asset は browser-local Settings が URL を保持するだけで、サーバから参照を削除する API / TTL はない。
  - generation request は相互に直列化するだけで、`record/prepare` と `record/start` は lock を待たない。両 endpoint はまず recording priority を同期的に予約して epoch を単調増加させる。state 確認後に待機していた request を含め、古い epoch で admission された generation は予約が既に release 済みでも拒否する。進行中の synthesis は sidecar の `/cancel` が worker process を終了し、cancel token も cache publish を中止する。cancel 呼び出しは 2 秒を上限として同期的に完了してから recorder request を続行する。
  - recorder が `armed` / `recording` / `stopping`、status が不明、または recording priority に preempt された場合は fail-closed に `deferred: true` と理由を返す。部分 asset を既に作れた場合は同じ response に残る。録画の API response / state / DDS 処理はこの生成の可否に依存しない。Collect は生成を発火せず、準備済み URL の browser playback のみを行う。
...
4. `POST /api/v1/record/stop {capture_id}` → recorder stop → 最終 metadata（`message_count` / `bytes` / `ended_at` / topics）を再同期して `state=completed`。確定後、**停止時クイックチェックを stop 応答の外で走らせ**（下記）、さらに **digest ジョブを投入**する（下記「digest ジョブ」）。

   **ブラウザ操作権（trusted LAN の認証ではない）**: start に成功したブラウザへ HttpOnly / SameSite の session cookie に不透明な操作トークンを発行し、通常 stop は body の `capture_id` とそのトークンが同じ capture を指すときだけ許可する。成功応答の消失に備え、その capture の完了後も次の start までは同一要求の冪等再送を許す。トークン欠落・不正・古い capture は **`409`**（`record_control_token_invalid` / `record_control_capture_mismatch`）で recorder を呼ばない。`GET /record/status` の additive な `control: {capture_id, controlled_by_this_client, lease_known}` は画面ごとの真実であり、他端末には「別のブラウザが操作中」だけを示す。別ブラウザは確認後に `POST /record/takeover {capture_id}` でトークンを回転して引継ぎ、それから通常 stop を使う。起動端末や orchestrator の再起動でメモリ上の lease が失われたときは所有権を推測せず `409 record_control_recovery_required` とし、確認済みの `POST /record/takeover {capture_id}` を優先する復旧経路とする。起動端末が使えない、または緊急に停止する必要がある場合は、別の確認済み `POST /record/force-stop {capture_id}` を使う。force-stop も recorder の実際の active `capture_id` を lock 内で照合するため、古い画面が新しい take を止めることはない。reconciliation / recorder 自動停止 / cleanup はブラウザ API を迂回する明示的な system-stop 境界を使う。
...
- **データモデル**:
  ...
  - `plan_catalog`（1 行テーブル）: `id`（`=1` CHECK）/ canonical Projects → Tasks → Conditions、Failure reasons、Operators、External controls の JSON `payload` / `updated_at` / 単調 `revision`。未設定は行なし（revision 0）。CAS 比較・更新は Store 内の同一 SQLite transaction で行う。`catalog/plan_catalog.json` sidecar も全 payload と revision を保持する。
- **エンドポイント**:
