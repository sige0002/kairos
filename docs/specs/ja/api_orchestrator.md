...
- Batch（**Collect の進行を永続化**）: `POST /api/v1/batches`、`PATCH /api/v1/batches/{id}`、`GET /api/v1/batches?status=&robot=&operator=&limit=&offset=`、`GET /api/v1/batches/{id}`。**`POST/PATCH /api/v1/episodes` は廃止** — episode が持っていた項目は capture 行そのものに載り、書き込みは `PATCH /api/v1/captures/{id}/review` が担う（下記「Batch」）。
