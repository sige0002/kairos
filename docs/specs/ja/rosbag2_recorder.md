...
- `POST /record/disarm { expected_capture_id }` — **原子的な compare-and-disarm**。session lock 内で現在の state が `armed` かつ `capture_id` が完全一致するときだけ disarm し、`{ disarmed: true, ...status, disarmed_capture_id }` を返す。start 済み（`recording` / `stopping`）、別 capture、idle/terminal では **何も変更せず** `{ disarmed: false, ...current_status }` を返す。これは通常の `stop` の代替ではなく、低優先度の pre-arm 解放だけに使う内部 API である。
