...
- ICE / network reachability:
  ...
  - The wait for ICE gathering before answering is `WEBRTC_ICE_GATHER_TIMEOUT_S` (default `5.0`). On expiry a partial answer is sent (host candidates suffice on a LAN), but through TURN a partial answer missing its relay candidate is a cause of the "connects but the video is black" family — raise this value in a TURN deployment on a slow uplink (expiry is logged at WARNING; 2026-08-11, sweep S4).
