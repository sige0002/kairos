...
- `POST /record/start` — body:
  ...
  ```
  → `201 { run_id, capture_id, state, started_at, arming? }`. The type of `topics` is `string[] | "all"`. `robot` is optional (falling back to `robot_name` in `RECORDING_CONFIG` when omitted). `collection_context` is the all-nullable `{ batch_id, batch_seq, project, task, condition, robot, operator }`, with unknown fields preserved. It is fixed in the manifest when **actual Start** succeeds; top-level `robot` / `operator` / `task` remain as before. If a matching `armed` session exists from just before, this becomes the fast path that only resumes it ([details](#recording-start-latency-many-topics-and-two-phase-start)).
  - **`qos_override_patterns`** (optional, added 2026-08): the pattern → QoS list of the orchestrator's **live** recording config (same shape as `recording.yaml`'s `topic_qos_overrides`). **Takes precedence over this recorder's own startup config** — a robot switch hot-swaps only the orchestrator's copy, and the recorder never re-reads its file until restart, so without this a post-switch recording was made with the previous robot's QoS while being labelled with the new robot's name (timing sweep S1-3). Resolution order: per-request exact name → **the request's pattern list** → the startup config's patterns → `qos_default`. **The list REPLACES the startup list rather than layering on it** (an empty list is itself the assertion "no overrides", not a fallback to the stale file). The orchestrator attaches it to every start as long as it has a config. It is also **part of the prepare/start match**: a config switched between prepare and start disarms the armed session and falls through to a full start — never resuming a session whose QoS was materialised under the previous config.
...
- `POST /record/disarm { expected_capture_id }` — **atomic compare-and-disarm**. Inside the session lock, it disarms only when the current state is `armed` and `capture_id` exactly matches, returning `{ disarmed: true, ...status, disarmed_capture_id }`. For a started session (`recording` / `stopping`), another capture, or idle/terminal state, it changes nothing and returns `{ disarmed: false, ...current_status }`. This is an internal API for releasing low-priority pre-arm only, not a replacement for normal `stop`.
...
- Errors: an unwritable `/data/objects` or insufficient free space rejects recording (`507`-equivalent). A duplicate start / duplicate prepare returns `409`. The `details` of a failed start's `507` **always includes the `capture_id`**. `failed_start_record_error` is attached only when writing the sidecar itself failed. **A failed `prepare` is the exception** (2026-08-11, sweep S2-7): the `507` + `capture_id` are the same, but **no `.failed.json` sidecar is written** (materialised temporaries like `.qos.yaml` are cleaned up). A prepare is the console's background keep-alive (30-second cadence); filing every attempt against a persistent arm blocker silently piled "Not usable" captures into the store — the durable record of the failure is the log, and the operator is told through the console's pre-arm degraded surface (→ capture_store §3.4).
...
2. `POST /record/start` — if an armed session exists AND the **spawn-affecting fields** (the normalised topic selection, `compression`, `split`, `qos_default`, `qos_overrides`) **match** the prepare-time request, this is the fast path: just call `~/resume` via the held clients (**no re-spawn, no discovery wait** — `start_delay_s`/`post_discovery_delay_s` are not re-applied either). If resume cannot be confirmed (the service disappeared, still paused after resume, etc.) it is treated the same as the existing fail-safe (terminate the process, delete the capture directory, `507 record_arm_failed`). `run_id` is not part of the match (it was already fixed at prepare time, so the committed `run_id` / `capture_id` are always the armed session's). `operator`/`task`/`collection_context` are not part of the match: they do not affect spawning, and both the manifest and a resume-failure failed-start record use the **actual Start request**, not prepare metadata. On a **mismatch**, the old armed session is disarmed (below; no failure record is written) and it falls back to the **same conventional full synchronous path** as when there was no armed session — a standalone `start()` still completes correctly on its own, as before.
