...
- `GET /jobs/{id}/result` returns **`artifacts` normalised to data-root-relative paths** (2026-07-15): dora_runner reports absolute container paths (e.g. `/data/report/<pipeline>/<run_id>/plot.png`), and the orchestrator relativises those under `data_dir`, so each artifact is directly fetchable via `GET /api/v1/files/{path}`. This is the **visualisation channel that lets a plugin surface images (plots etc.) with zero UI edits** ([dora_plugins.md §2.5](dora_plugins.md)). Absolute paths outside `data_dir` and already-relative paths pass through unchanged.
