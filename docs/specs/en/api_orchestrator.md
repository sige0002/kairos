...
- Import (external bags): **`GET /api/v1/imports/scan?path=<dir>` (2026-08-05)** — scans a folder SHALLOWLY (the path itself if it is a bag, otherwise its immediate children) and returns each candidate with `importable` and, when it is not, the reason (`already_imported` / `import_no_metadata` …) plus a remedy. It exists so an operator sees what will and will not come in **before a single byte is copied** (a recursive walk of an operator-supplied path would be an unbounded crawl, hence shallow). `POST /api/v1/imports` (body `{ source_path, move? }` → `202 { import_id }`. The source is **a path on the server** [a bag is several GB and is not something to upload through a browser]. Validation is synchronous, the copy is asynchronous), `GET /api/v1/imports`, `GET /api/v1/imports/{id}`
