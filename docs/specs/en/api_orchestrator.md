...
- Datasets: `GET /api/v1/datasets` (a list scanned from `data/<operator>/<task>/<NNN>/dataset.json`. Reads only under `data_dir`), `GET /api/v1/datasets/{operator}/{task}/{index}` (**detail of an exported dataset**. See "Dataset export" below), `POST /api/v1/datasets/export` (body `{ run_id }`), `POST /api/v1/datasets/export-all` (**bulk** export of completed runs in `recorded/`)
