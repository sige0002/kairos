...
- Batch (**persists Collect's progress**): `POST /api/v1/batches`, `PATCH /api/v1/batches/{id}`, `GET /api/v1/batches?status=&robot=&operator=&limit=&offset=`, `GET /api/v1/batches/coverage?task=`, `GET /api/v1/batches/{id}`. **`POST/PATCH /api/v1/episodes` are removed** — the items an episode used to carry now live on the capture row itself, and writes go through `PATCH /api/v1/captures/{id}/review` (see "Batch" below).
...
- `GET /api/v1/datasets/{dataset_id}/archive` — progress. The durable fields (status / destination / archive_started_at / archived_at) come from the rows and survive a restart; `running` / `current_capture_id` / `current_bytes` / `error` are process memory and are honestly reset. **`archiving` with `running: false` is "resumable"**, and the UI renders it as Resume. `error.code` carries why the runner stopped (`ledger_unreadable` = the ledger could not be read, so the run could decide nothing and halted; it is a run halt rather than a refusal before starting, which is why it is not in the enumeration above — others include `bytes_unaccounted` / `capture_not_found` / `internal_error`). It is a separate endpoint rather than an extension of `GET /datasets/{id}` so that 1-second polling does not thrash the detail cache.
...
- **Endpoints**:
  ...
  - `GET /api/v1/batches/coverage?task=<required>` — **per-condition coverage for one task, aggregated in SQL**. The response is `{ task, rows: [{ condition, recorded, is_floor }] }`. `recorded` is the **sum** of `episodes_recorded` (the monotone counter) over every batch of that task and condition; `is_floor` is **true as soon as one term is a floor** (a sum becomes a lower bound — §8.2 rule 6; there is no way to say which part is uncertain, so the flag rides on the sum, not per batch). Batches whose `condition` is `NULL`, empty, or **`—` (the console's "unset" placeholder burnt into the catalog; E-5)** are excluded. Rows are sorted by `condition`. **A missing (or empty) `task` is `422`** — the aggregation is only ever used per task; a cross-task total adds unrelated work into one number. If an all-tasks aggregate is ever needed, it gets its own endpoint. **Why paging the list was not the fix**: Collect's Coverage card used to pull `GET /api/v1/batches` unfiltered and sum client-side — 817 KiB every 30 seconds at 5000 batches (E-27). A default limit would silently truncate a total presented as complete (the same reason the list has no default limit), so **the aggregation itself moved server-side**. **The ∪ with the plan vocabulary (listing zero-recording conditions as 0 rows) stays the client's job**: the server returns **only observed aggregates** — the plan catalog is client-side vocabulary, and rows minted from it would report a plan, not a measurement.
