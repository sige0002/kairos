...
- **A capture that is a dataset member is refused for archive too** (the same `400` as delete). It would leave the `views/` symlink dangling, so remove the membership first.
