...
- `POST /api/v1/datasets/{dataset_id}/archive` — body `{ destination?, reason? }` → **`202`** (doubles as start and resume). The server composes `<destination>/<operator>/<task>/<name>` (the components sanitised the same way as views).
  ...
  - Resume: with status `archiving` and no run in flight, a re-POST resumes idempotently from where it left off. The destination is **either omitted or identical to the record** (otherwise `409 archive_destination_mismatch`). While a run is in flight, `409 archive_in_progress`.
