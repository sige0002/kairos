...
- `POST /api/v1/datasets/{dataset_id}/archive` — body `{ destination?, mode?, reason? }` → **`202`** (doubles as start and resume). `mode` is `move` (the default; deletes the sources; demands exclusive members) or `copy` (seal only; the sources untouched; shared members legal — the standard for a combined set). The server composes `<destination>/<operator>/<task>/<name>` (the components sanitised the same way as views). copy also runs under `delete_unavailable` (it deletes nothing).
