...
| `KOKORO_MEMORY_LIMIT` | `2g` | Memory limit for the opt-in Kokoro container |
...
| `CYCLONEDDS_URI` | (optional) | Cyclone DDS configuration file URI. On a shared LAN use `file:///config/cyclonedds-shared-domain.xml`; use `file:///config/cyclonedds-localhost.xml` only when closing DDS to localhost behind a Zenoh gateway. Passed to the container via `env_file` (the ROS services mount `/config` read-only) |
...
**For the sample-bag replay harness** (`deploy/test/compose.yaml`, read by `make rosbag` / `make rosbag-loop`; not passed to the 8 core services):
...
- A normal compose startup gives seven services a healthcheck based on `GET /healthz` (frontend uses the nginx root), and frontend waits for the orchestrator to become healthy via `depends_on: orchestrator (service_healthy)` before starting. The Kokoro 82M sidecar uses about 1.5 GiB of resident memory and is opt-in through the `audio` profile; it starts as the eighth service only with `COMPOSE_PROFILES=audio make up`. It is not an orchestrator startup dependency and does not gate recording; if it stops, only Voice degrades gracefully. Recording start terminates the inference worker to release CPU and model memory, and the next Prepare reloads it. The model config, weights, and English/Japanese voice subset are pinned by revision and SHA-256 at image build time, and runtime downloads are disabled. Installations that save/load images with the audio profile can operate the recording PC offline after one online build. Kokoro upstream code and model are Apache-2.0 and the image includes license/NOTICE files (this is not a substitute for product-wide legal review; distribution must also audit dependency licenses).

### Switching the DDS implementation (Fast DDS ↔ Cyclone DDS)

Fast DDS and Cyclone DDS may interoperate at the DDS/RTPS level, but behavior for discovery, QoS, locator selection, and vendor-specific transports depends on the configuration. Check connectivity and load on the actual robot graph, and apply the following safety profile as well when selecting Cyclone DDS for Kairos.

- The images for the four ROS services (recorder / monitor / streamer / probe) **bundle both RMWs** (Fast DDS and Cyclone DDS). They can be switched in `.env`, but **do not change the global default**. The normal setup continues to use `RMW_IMPLEMENTATION=rmw_fastrtps_cpp` (Fast DDS).
- Also make sure **`ROS_DOMAIN_ID` matches the robot** (default `0`).
- When using Cyclone DDS on a normal shared LAN, set `RMW_IMPLEMENTATION=rmw_cyclonedds_cpp` and `CYCLONEDDS_URI=file:///config/cyclonedds-shared-domain.xml` on **all Kairos processes participating in Cyclone**. This profile leaves multicast only for participant discovery via `AllowMulticast=spdp`, and uses unicast for endpoint discovery and user data. `ParticipantIndex=none` also avoids finite participant-index port allocation for many processes on the same host and domain.
- `config/cyclonedds-localhost.xml` is dedicated to the Zenoh gateway configuration and pins DDS to `lo`; do not substitute it for the shared-LAN profile. With localhost pinned, remote RViz2 or plain ROS subscribers cannot join DDS directly and must go through the gateway. The shared-LAN profile, by contrast, leaves SPDP discovery on the LAN, so it discovers remote participants on the same `ROS_DOMAIN_ID` and delivers topic data by per-peer unicast.
