...
| `TTS_PROVIDER` | `espeak-ng` | The local voice provider for Settings > Audio. Select `espeak-ng` (lightweight, default), `voicevox` (optional high-quality Japanese), or `disabled`. Writing exactly `TTS_PROVIDER=voicevox` in `.env` or `.env.split` makes the Makefile automatically append `compose/voicevox.yaml` to build / up / images-save. Provider selection is fixed when the orchestrator starts, so switching requires a restart |
| `TTS_VOICEVOX_URL` | `http://127.0.0.1:50021` | The VOICEVOX Engine API as seen by the orchestrator. The default overlay binds only to host-network loopback; browsers never access this port directly |
| `TTS_VOICEVOX_CPUS` | `2` | CPU cap for the optional VOICEVOX container. This is an additional recording-priority guard; advance synthesis itself is also cancelled / deferred by recording priority |
...
- compose gives all 7 core services a healthcheck based on `GET /healthz` (frontend uses the nginx root), and frontend waits for the orchestrator to become healthy via `depends_on: orchestrator (service_healthy)` before starting. VOICEVOX is an optional eighth sidecar, built as the local `kairos-voicevox` image from a digest-pinned multi-arch image. It is automatically included in `make images-save` / `images-load`, so after the first build in an online environment, the recording PC can operate offline. Its use must comply with the VOICEVOX and individual voice-library terms, and the UI displays the selected `VOICEVOX:<speaker> · <style>`.
