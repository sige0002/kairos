...
| `KAIROS_ARCHIVE_ROOTS` | (optional, default empty=disabled) | The roots permitted as an archive destination for captures / datasets (`:`-separated, **absolute paths as the CONTAINER sees them**). Empty = the archive feature is not offered at all (we do not put up a button that is guaranteed to fail). A destination is validated against this list, and anything overlapping `data_dir` is rejected ([capture_store](capture_store.md) §6/§6.1). **Always set it together with the volume mount** — single-host: add `COMPOSE_FILE=compose.yaml:compose.archive.yaml` + `ARCHIVE_DIR=<host path>` to `.env` (the root `compose.archive.yaml` mounts `${ARCHIVE_DIR}` at `/archive`). Allowing an unmounted root lands exports **in the container's filesystem layer, gone on the next recreate** (and a move has deleted the sources by then). The split deploy needs no `COMPOSE_FILE` line — **`make recording-up` appends `-f compose.archive.yaml` by itself whenever the split env sets `ARCHIVE_DIR`** (an explicit `-f` beats `COMPOSE_FILE`, so the Makefile owns that wiring). Plain `docker compose -f compose.recording.yaml` users add `-f compose.archive.yaml` on the command line |
