...
For `loss_report`, each above-threshold interval is also listed in `events` with its topic, capture-relative `start_offset_ms` / `end_offset_ms`, `gap_ms`, median ratio `gap_ratio`, and median-cadence-based `estimated_missing`. Event positions use capture-relative MCAP `log_time` so topics are comparable, while the gap verdict uses the selected clock described below. `estimated_missing` is an estimate from the long interval between two recorded messages, not a confirmed count.
...
- **`clock_check`** — robot-independent **recorder-host vs publisher clock consistency check** (no ROS; direct MCAP read). The two times in a bag are stamped by different machines' clocks — `header.stamp` by the publisher (the robot side in a split deployment), the MCAP `log_time` by the recording host on receipt — so their difference should be transport + queueing latency (milliseconds); a wrong recorder clock turns it into the clock offset (**negative** when the recorder runs behind = "received before published"). Method: ① a decode-free sweep collects the **selected topics'** `(log_time, publish_time)` pairs and every channel's encodings; where `publish_time` is trustworthy (the same `source_times` rule as loss_report) the **median `log_time − publish_time`** (the **same orientation** as the header offset) is reported as a decode-free cross-check. ② a per-topic bounded decode samples `header.stamp` — the first `n_head` messages and, **read in reverse so the tail is genuinely the bag's end**, the last `n_tail` (`max_samples_per_topic` total, default 200). A topic small enough to fit the budget is read once in full (stats over every message) and the step verdict compares its **first and last quarters** (disjoint — `count` can never exceed `message_count`). Decode cost is bounded per topic by `min(max_samples_per_topic, message_count)`. Non-CDR/ros2msg channels (an imported foreign bag's json etc.) are skipped with a reason, never fatal to the job. Verdicts: `offset_suspected` when `|median| > threshold_ms` (default 500); `step_suspected` when the head and tail medians disagree by more than the threshold (the shape of a **mid-recording NTP step**, which also carves a fake gap / time reversal into `log_time`). A flagged topic is classified by `offset_kind` — **`clock_step`** (the head and tail windows disagree; with a step the blended median's classification would be arbitrary, so the step itself is the diagnosis) / `clock_disagreement` (the header offset matches the clock offset the DDS stamps measure = the recorder clock itself is off) / `source_stamping` (the DDS stamps prove the clocks agreed and the header stamps themselves are foreign/late — **a re-recorded bag REPLAY lands here**: the replay carries the original session's header stamps) / `indeterminate` (no trustworthy publish_time to tell the two apart). Headerless types and zero stamps are reported with a reason and **never flagged** (a zero stamp compared to epoch must not fabricate a 56-year offset); a partly unstamped topic is disclosed via `unstamped_sampled`. The summary carries `result: pass|fail` (fail = at least one flag) so the Validation tab's generic renderer can gate on it, and **states in `note` when nothing was checked** (an empty bag, or globs matching no topic) — never a silent green vouching for a bag it did not look at. A definition also states this is an inference, not a measurement of which clock is right. **An MCAP without a chunk index cannot be read in reverse**, so over-budget topics degrade honestly to head-only sampling (tail `null`, stated in the summary `note`) with step detection disabled (topics within the budget are read in full and keep it). Median-window step detection **can miss a step landing in roughly the last tenth of a recording** (stated in the definition). Params: `threshold_ms` / `max_samples_per_topic` / `target_topics` (globs). Report: `data/report/clock_check/<capture_id>/summary.json`.
- **`video_check`** — on-demand (params `{topic}`) `CompressedImage`→mp4 preview. Generated with PyAV (`av` + `Pillow`), which are **lazily imported** so the service can start even when the packages are absent (when absent it becomes a clearly failed job). Output is `data/report/video_check/<capture_id>/<topic>.mp4`, served via `GET /api/v1/files/...`. The encode cap is params `max_frames` (**`0` = every frame**). The default comes from the `VIDEO_MAX_FRAMES` env (unset = `0` = the full episode — reviewers watch the whole take, the 2026-08-07 decision; set a cap only where encode time/disk actually hurts. A broken value falls back to "everything" — a preview that silently stops short is worse than a slow one). The fps-estimate cadence sample stays bounded independently of the cap (`FPS_SAMPLE_FRAMES`). The encoder is env-tunable — `VIDEO_ENCODE_THREADS` (default 4: one preview must not saturate the box) / `VIDEO_ENCODE_PRESET` (default medium = x264's own; **the defaults trade no quality** — this video is also what an operator judges camera health from, and codec mush is indistinguishable from a degraded camera) / `VIDEO_ENCODE_CRF` (default 23). Measured (720p worst-case noise, 300 frames): threads 1→4 takes 14.7s→7.5s at identical quality; dropping to preset veryfast reaches 4.6s. A summary cut off at the cap carries `truncated: true` and the real total message count, and the UI shows a "head only" label plus a **Re-encode full episode** button (re-posting `{force: true, max_frames: 0}`). The playback fps is estimated from the frame-time cadence under the same rule as loss_report — **`publish_time` preferred, `log_time` fallback** (the clock used is stated as `fps_time_source` in the summary). The mp4 is encoded to a temp file and atomically renamed, so a re-encode that fails midway never corrupts an mp4 being served. The (capture_id, topic) cache is cap-aware (a truncated cache misses a full-length request; an untruncated one within the requested cap hits).
...
- **`params.dataset_dir` is retired** (v2) — now that a dataset is logical and a recording's bytes never move, the need to switch between "where it is while recording" and "where it is after export" is gone. `loss_report` / `clock_check` / `video_check` / `signal_report` all read `objects/<capture_id>`, and outputs / caches are fixed at `data/report/<pipeline>/<capture_id>/`. Putting a capture into a dataset or taking it out leaves both the reports and the mp4 cache valid as they are.
