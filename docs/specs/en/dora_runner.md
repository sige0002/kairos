...
- **`clock_check`** — robot-independent **recorder-host vs publisher clock consistency check** (no ROS; direct MCAP read). The two times in a bag are stamped by different machines' clocks — `header.stamp` by the publisher (the robot side in a split deployment), the MCAP `log_time` by the recording host on receipt — so their difference should be transport + queueing latency (milliseconds); a wrong recorder clock turns it into the clock offset (**negative** when the recorder runs behind = "received before published"). Method: ① a decode-free full sweep collects every topic's `(log_time, publish_time)` pairs and the recording window, and — where `publish_time` is trustworthy (the same `source_times` rule as loss_report) — reports the median `publish_time − log_time` as a **decode-free cross-check** (a large header offset with a small publish offset points at late stamping on the publisher, not at the clocks). ② a sampled decode over a **head window and a tail window** of the bag (up to `max_samples_per_topic`, default 200) computes per-topic median/p05/p95/min/max of `log_time − header.stamp` and the negative share (**never a full-bag decode** — bounded by the windows and quotas). Verdicts: `offset_suspected` when `|median| > threshold_ms` (default 500); `step_suspected` when the head and tail medians disagree by more than the threshold (the shape of a **mid-recording NTP step**, which also carves a fake gap / time reversal into `log_time`). Headerless types and zero stamps are reported with a reason and **never flagged** (a zero stamp compared to epoch must not fabricate a 56-year offset). The summary carries `result: pass|fail` (fail = at least one flag) so the Validation tab's generic renderer can gate on it, plus a definition stating this is an inference, not a measurement of which clock is right. Params: `threshold_ms` / `max_samples_per_topic` / `target_topics` (globs). Report: `data/report/clock_check/<capture_id>/summary.json`. Note that **a capture made by re-recording a bag REPLAY always fails** (the replay carries the original session's header stamps) — a correct detection that surfaces such captures before they blend into training data.
