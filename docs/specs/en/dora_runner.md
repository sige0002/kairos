...
- **`loss_report`** — robot-independent, config-free per-topic loss estimation. From the message times of a completed MCAP, it computes the **median interval** per topic and derives `loss ≈ 1 − actual/expected` (read-only, does not decode payloads). The clock **prefers the sender-side `publish_time` (DDS source timestamp)**, falling back to the receive-side `log_time` only when the bag did not record one (older rosbag2: every message has `publish_time == log_time`, or 0) — keeping receive-side jitter (DDS transport, recorder scheduling/cache) out of the cadence estimate. **Which clock produced the numbers is stated per topic as `time_source`** (`"publish_time"` / `"log_time"`; honesty rule). Report: `data/report/loss_report/<run_id>/summary.json`.
