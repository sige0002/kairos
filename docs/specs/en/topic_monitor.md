...
- **ROS2 Subscribers** — use rclcpp serialized subscriptions by default. Arrival times, byte counts, and DDS lost counts are retained in C++ without passing payloads to Python. Subscribe to **the allowlist only** (no bulk subscription to all topics). The existing rclpy node handles graph discovery and QoS resolution.
...
## Build and receive backend

- `make build monitor` also builds the C++ shared library and bundles it in the same image. No compilation or additional downloads occur at startup.
- C++ reception and aggregation are the default for normal operation; users do not need to select a low-load mode. Monitored topics, received message counts, and the approximately one-second SSE update cadence are not reduced.
- Topic names and robot names are not hardcoded. Additional topics matching concrete names or globs in `default_topics` are detected on the existing approximately two-second discovery cycle and subscribed to according to their type and QoS.
- Custom messages use the `install/` generated by the existing `make msgs-build` as the existing overlay, without changes. The normal message build also includes the C++ artifacts, so no separate message build step is required.
- For maintenance, set `KAIROS_MONITOR_BACKEND=python` in `.env` to return to the previous implementation (takes effect when the container is recreated). The default is `native`. Native initialization failures do not silently fall back to Python; they are reported in logs and through a not-ready response from `/readyz`.
- C++ reception uses Jazzy's event-notification executor. Notifications without payloads are retained as counts by source and type and processed one at a time. For a fixed set of notification sources, the number of queue elements does not grow in proportion to the notification count. The native Monitor does not use timer events; the wait while checking the stop flag is at most 100ms, excluding processing time.
- The C++ and Python ABI versions and snapshot structure sizes are checked at startup. Do not mix shared libraries from different versions.
- The native path bypasses the Python sample callback, so `self_load.callback_lag_ms` / `callback_lag_p95_ms` are `null`. Unmeasured processing time is not displayed as 0 ms. Payload stamps are not decoded; `stamp_delay_ms` and `sensor_preview` are also `null`.
